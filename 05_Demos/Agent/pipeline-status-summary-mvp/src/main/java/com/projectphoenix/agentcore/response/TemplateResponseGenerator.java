...
public final class TemplateResponseGenerator implements ResponseGenerator {
    ...
    public String generate(UserRequest request, SkillResult result) {
        if (result.evidence() == null) return String.join("；", result.messages());

        EvidenceBundle bundle = result.evidence();
        String facts = bundle.evidenceItems().stream()
                .map(item -> item.factName() + "=" + item.factValue() + " [" + item.sourceTool() + "." + item.sourceField() + "]")
                .collect(Collectors.joining("；"));
        String missing = bundle.missingEvidence().isEmpty() ? "" : "；缺失证据：" + String.join("；", bundle.missingEvidence());
